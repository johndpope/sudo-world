...
extension UIView {
    ...
    @discardableResult
    func constrainWithinLayoutGuide(of viewController: UIViewController) -> [NSLayoutConstraint] {
        let topConstraint = constrainTopToTopLayoutGuide(of: viewController)
        let horizontalConstraints = constrainEdgesHorizontally(to: viewController.view)
        let bottomConstraint = constrainBottom(to: viewController.view)
        return [topConstraint] + horizontalConstraints + [bottomConstraint]
    }
    
    @discardableResult
    func constrainTopToTopLayoutGuide(of viewController: UIViewController) -> NSLayoutConstraint {
        let topConstraint: NSLayoutConstraint
        if #available(iOS 11.0, *) {
            topConstraint = topAnchor.constraint(equalTo: viewController.view.safeAreaLayoutGuide.topAnchor)
            topConstraint.isActive = true
        } else {
            topConstraint = topAnchor.constraint(equalTo: viewController.topLayoutGuide.bottomAnchor)
            topConstraint.isActive = true
        }
